%----------------------------------------------------------
% Document class
%
\documentclass[12pt,a4paper,oneside,final,notitlepage]{article}%

%Geometry
%\usepackage[a4paper,left=2cm,right=2cm,top=4cm,bottom=4cm,headsep=2cm,footskip=2cm]{geometry}
%\usepackage[a4paper,left=2cm,right=2cm,top=4cm,bottom=4cm]{geometry}
\usepackage[a4paper,margin=2cm]{geometry}

\renewcommand{\topfraction}{0.9}
\renewcommand{\textfraction}{0.1}
\renewcommand{\floatpagefraction}{0.9}

%\usepackage[bf,medium,raggedright]{titlesec}
%\titleformat{\chapter}{\bfseries\Large}{\thechapter.}{.5em}{}
%\titlespacing{\chapter}{0pt}{*1}{*1}

%Package for todonotes
%\usepackage[disable]{todonotes}
%\usepackage{todonotes}

\usepackage{url}

\usepackage{xcolor, soul}
\setstcolor{red}
\sethlcolor{green}
\usepackage{listings}
\lstset{
language=[ISO]C++,
columns=flexible,
identifierstyle=\itshape,
keywordstyle=\bfseries\color{green!40!black},
commentstyle=\itshape\color{purple!40!black},
identifierstyle=\color{blue},
stringstyle=\color{brown},
morekeywords=[1]{
expects,ensures,assert,axiom,audit
}}

\newcommand\redout{\bgroup\markoverwith
{\textcolor{red}{\rule[.5ex]{2pt}{0.4pt}}}\ULon}

\newcommand{\remove}[1]{ {\setstcolor{red} \st{#1}} }

\newcommand{\add}[1]{ {\sethlcolor{green} \hl{#1}}}

\newcommand{\cppkey}[1]{%
{\color{green!40!black}\texttt{#1}}%
}

\newcommand{\cppid}[1]{%
{\color{blue}\texttt{#1}}%
}

\newcommand{\textmark}[1]{%
{\color{brown}\textbf{#1}}%
}

\usepackage{enumitem}

\begin{document}

\input{config}

\noindent
\textbf{Document Number}: \texttt{\paperid}\\
\textbf{Date}: \paperdate\\
\textbf{Reply to}: J. Daniel Garcia\\ 
\textbf{e-mail}: \url{josedaniel.garcia@uc3m.es}\\
\textbf{Audience}: EWG / CWG\\

\title{Avoiding undefined behavior in contracts}
\author{J. Daniel Garcia\\
Computer Science and Engineering Department\\
University Carlos III of Madrid
\and
Ville Voutilainen\\
Qt Company
}
\date{}

\begingroup
\let\newpage\relax% Void the actions of \newpage
\maketitle
\endgroup

\input{contents}

\bibliographystyle{unsrt}
\bibliography{contracts}

\end{document}
